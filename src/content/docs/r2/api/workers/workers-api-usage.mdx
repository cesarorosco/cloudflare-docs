...
When you use R2 buckets in local development mode, you don't directly interact with a physical directory on your local machine. Instead, Wrangler creates a virtual R2 bucket in local storage.
Key Points:
- No direct Directory Mapping: There's no specific directory where you place files to be directly accessible as an R2 bucket in local mode.
- Wrangler Handles Storage: Wrangler manages the storage and retrieval of objects in the virtual R2 bucket.
- Interacting with the Bucket: You interact with the bucket using the R2 API within your Worker code.
- Local Persistence: Wrangler stores the local R2 bucket data in a .wrangler/state directory.
